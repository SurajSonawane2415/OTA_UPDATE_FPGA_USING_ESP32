...
esp_loader_error_t flash_binary(const uint8_t *bin, size_t size, size_t address)
{
    ...
    err = esp_loader_flash_start(address, size, CHUNK_SIZE); // Use CHUNK_SIZE for payload size
    if (err != ESP_LOADER_SUCCESS) {
    ...
    while (size > 0) {
        size_t to_read = MIN(size, CHUNK_SIZE);
